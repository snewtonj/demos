...
if(strlen($token) == 40) {//test the length of the token; it should be 40 characters
  ...
  if ($auth_info['stat'] == 'ok') {
    ...
    echo "<h2>Welcome " . $auth_info['profile']['name']['formatted'] . "!</h2>";
    ...
    if ($engage_pro === true){
      ...
      if ($provider == 'Google' || $provider == 'Twitter' || $provider == 'Facebook' || $provider == 'Yahoo!'){
        ...
        $url = 'https://rpxstaging.com/api/v2/get_contacts';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
...
// ),
        ...
        $url = 'https://rpxstaging.com/api/v2/activity';
        $curl = curl_init();
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($curl, CURLOPT_URL, $url);
        curl_setopt($curl, CURLOPT_POST, true);
